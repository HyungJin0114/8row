...
class OrderService {
  ...
  createOrder = async (userId, storeId, price, menuIdList, countList) => {
    try {
      ...
      try {
        const checkStoreData = await this.orderRepository.checkStore();

        if (!checkStoreData) {
          return '404, 해당 점포를 찾을 수 없습니다.';
        }
        ...
        return `200@"massage": "주문이 완료되었습니다."`;
      } catch (error) {
        await t.roollback();
        return `400@"massage": "주문에 실패했습니다."`;
      }
    } catch (error) {
      return `400@"massage": "주문에 실패했습니다."`;
    }
  ...
  deleteOrder = async (userId, storeId, orderId) => {
    try {
      const deleteOrderData = await this.orderRepository.compareOwner(storeId);

      if (deleteOrderData.ownerId != userId) {
        return `400@"massage": "점포의 소유주가 아닙니다."`;
      }

      await this.orderRepository.deleteOrder(orderId);

      return `200@"massage": "주문이 취소되었습니다."`;
    } catch (error) {
      return `400@"massage": "주문 취소에 실패했습니다."`;
    }
  };
  ...
  getOrder = async (userId, storeId) => {
    try {
      const deleteOrderData = await this.orderRepository.compareOwner(userId);
      ...
      return `200@${getOrderData}`;
    } catch (error) {
      return `400@"massage": "주문 내역 불러오기에 실패했습니다."`;
    }
  };
  ...
  completeOrder = async (userId, storeId, orderId) => {
    try {
      ...
      return `200@"massage": 배달 완료 처리에 성공했습니다.`;
    } catch (error) {
      return `400@"massage": "배달 완료 처리에 실패했습니다."`;
    }
  };
}
